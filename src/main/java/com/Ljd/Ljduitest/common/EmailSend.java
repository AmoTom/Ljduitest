...
public class EmailSend {
		...
		public void SendEmail(String file){
			...
			Session session = Session.getDefaultInstance(props, new javax.mail.Authenticator() {
				...
				return new PasswordAuthentication("549666454@qq.com", "dqpzthwztiaxbeih");
				}
			});
			...
			try {
				...
				// 设置发件人邮箱地址
				message.setFrom(new InternetAddress("549666454@qq.com"));

				// 设置收件人邮箱地址
				message.setRecipients(Message.RecipientType.TO,InternetAddress.parse("dan_zhiwei@163.com"));

	            // 设置邮件主题
				message.setSubject("测试报告邮件发送");
